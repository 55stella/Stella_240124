...
public class Login  extends TestBase {
   ...
   public void initializer(){
    ...
    @Test(priority = 1, description = "Validate that the User can Launch the Qa App.")
    ...
    @Test(priority = 2, description = "Validate that the Log in Button is Displayed  Once App is Launched .")
    public void TC5_ValidateLoginBtnIsDisplayed(Method method){
        startTest(method.getName(), method.getAnnotation(Test.class).description(), testCategory);
        validateText(landingPage.returnLoginTextInDashboard(), "Log In");
    }

    @Test(priority = 3, description = "Validate that the Login Button is clickable")
    ...
    @Test(priority = 4, description = "Confirm that the Password length cannot be Less than Five (5)Characters.")
    ...
    @Test(priority = 5, description = "Validate that the Username Follows email format Only.")
    ...
    @Test(priority = 6, description = "Validate that the username Only follows allowed Character Restrictions.")
    ...
    @Test(priority = 7, description = "Validate that User Cannot Log In with Email address that does not follow Email format")
...
//
    @Test(priority = 8, description = "Validate that the User cannot Proceed to Log in with empty Values for Username and Password.")
    ...
    @Test(priority = 9, description = "Validate that the User cannot Log in with an Invalid Username and Password.")
    ...
    @Test(priority = 10, description = "Confirm that the log in button is disabled until both the username and password fields are filled.")
    ...
    @Test(priority = 11, description = "Validate that User can Login with a Valid Username and Password.")
